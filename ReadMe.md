...
python Generate.py -l myset -s /.../songsfolder -t ./targetfolder -r /.../arial.ttf -b /.../arialbd.ttf -i /.../ariali.ttf
```
...
- The script takes the mp3-files from the folder given by `--songsfolder` (`-s`).
...
- The filenames of the png-files are random but the filename and the color for 2 files will be equal if the year, the artists and the songname is the same.
